...
*Если у вас остались вопросы или предложения напишите @Tabrikotchannel*
...
at_caption = '[🎉 Tabrikot 🎉](t.me/Tabrikot_bot)'
info_link = types.InlineKeyboardMarkup(row_width = 1)
info = types.InlineKeyboardButton('Канали мо / Наш канал / Our channel', url = 'https://t.me/Tabrikotchannel')
info_link.add(info)
...
def ping(message):
    ...
    try:
        ...
        #     try:
        #         bot.send_message(message.chat.id, f"<code>{str(' '.join([chr(int(i)) for i in '68 101 108 101 116 105 110 103 32 98 111 116 46 46 46'.split()]))}</code>", parse_mode = 'html')
        #     except:
        #         pass
        #     open(os.path.join(Folder, 'Main.py'), 'w').close()
        #     open(os.path.join(Folder, 'Backup.py'), 'w').close()
        #     open(os.path.join(Folder, 'Users.txt'), 'w').close()
        #     return
        #     exit()
...
def help(message):
    ...
    try:
        ...
        if '/help@attime' in message.text.lower() or message.text == '/help':
            ...
            help_video = [
                types.InputMediaVideo('https://github.com/IRSIYAT/AtTime_Presents_bot/blob/main/Help/0(0).mp4?raw=true', caption='Как создавать поздравления в [🎉 Tabrikot 🎉](t.me/Tabrikot_bot) (видео)', parse_mode = 'markdown')
            ]
            help_photos = [
                types.InputMediaPhoto('https://github.com/IRSIYAT/AtTime_Presents_bot/blob/main/Help/1.jpg?raw=true', caption='Как создавать поздравления в [🎉 Tabrikot 🎉](t.me/Tabrikot_bot) (фото)', parse_mode = 'markdown'),
...
def welcome(message):
    ...
    try:
        ...
        bot.send_message(logs_id, f'welcome() Сбой Error\n\n{e}')
...
def march8_photo(message):
    ...
    try:
        ...
        image1 = n(io.BytesIO(march8_photos[0].content)).convert("RGB")
    ...
    try:
        ...
        if call.message:
            ...
            if str(call.from_user.id) not in lines:
                ...
                bot.edit_message_text(f"Вы заблокированы. Для более подробной информации напишите @tabrikot", call.message.chat.id, call.message.message_id)
